    ...
    methods
        ...
        function list_map_as_text(obj, fid)
            ...
            if nargin<2
				...
				fprintf(fid,'\n<table cellspacing="15" cellpadding="2" width="1200px" height="100px"><tbody>');
				fprintf(fid,'\n<tr><td style="background-color: #CEF6CE;" width="400px">%s: %d Castles<br/>%s<br/>%d Barrels:',house_index2name(3),obj.castles_occupied(3),repmat('&#9819',1,obj.current_crowns(3)),obj.current_barrels(3));fprintf(fid,'%d',supply2(obj.current_barrels(3)));
				fprintf(fid,'\n</td><td style="background-color: #F6CECE;" width="400px">%s: %d Castles<br/>%s<br/>%d Barrels:',house_index2name(4),obj.castles_occupied(4),repmat('&#9819',1,obj.current_crowns(4)),obj.current_barrels(4));fprintf(fid,'%d',supply2(obj.current_barrels(4)));
				fprintf(fid,'\n</td><td style="background-color: #848484;" width="400px">%s: %d Castles<br/>%s<br/>%d Barrels:',house_index2name(5),obj.castles_occupied(5),repmat('&#9819',1,obj.current_crowns(5)),obj.current_barrels(5));fprintf(fid,'%d',supply2(obj.current_barrels(5)));
				fprintf(fid,'\n</td></tr><tr><td style="background-color: #F6E3CE;" width="400px">%s: %d Castles<br/>%s<br/>%d Barrels:',house_index2name(6),obj.castles_occupied(6),repmat('&#9819',1,obj.current_crowns(6)),obj.current_barrels(6));fprintf(fid,'%d',supply2(obj.current_barrels(6)));
				fprintf(fid,'\n</td><td style="background-color: #F5F6CE;" width="400px">%s: %d Castles<br/>%s<br/>%d Barrels:',house_index2name(2),obj.castles_occupied(2),repmat('&#9819',1,obj.current_crowns(2)),obj.current_barrels(2));fprintf(fid,'%d',supply2(obj.current_barrels(2)));
				fprintf(fid,'\n</td><td style="background-color: #E6E6E6;" width="400px">%s: %d Castles<br/>%s<br/>%d Barrels:',house_index2name(1),obj.castles_occupied(1),repmat('&#9819',1,obj.current_crowns(1)),obj.current_barrels(1));fprintf(fid,'%d',supply2(obj.current_barrels(1)));
				fprintf(fid,'\n</td></tr></tbody></table>');
                return;
            end
        ...
        function list_map_as_html(obj,map_count)
            ...
            %history of the map
            newline = sprintf('\n');
            map_history = urlread('file:///C:/Users/user/Documents/GitHub/GoT/testing_dairy.txt');
            map_history=strrep(map_history,newline,'<br/>');
            
            fprintf(h3dfile,'\n<div class="container">');
            fprintf(h3dfile,'\n<button type="button" class="btn btn-info" data-toggle="collapse" data-target="#demo">Map History</button>');
